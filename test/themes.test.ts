...
import { getTheme, getThemeSummaries, isValidThemeSlug, listThemes } from "../src/themes/index.ts";
...
Deno.test("theme registry lists all themes", () => {
  const themes = listThemes();
  assert.ok(themes.length >= 11, `Expected at least 11 themes, got ${themes.length}`);
});

Deno.test("theme registry includes vanilla as the first theme", () => {
  const themes = listThemes();
  assert.equal(themes[0].slug, "vanilla");
});

Deno.test("theme registry getTheme returns the correct theme", () => {
...
Deno.test("theme registry getTheme throws for unknown slugs", () => {
  assert.throws(() => getTheme("nonexistent"), /Unknown theme/);
});

Deno.test("theme registry all themes have required fields", () => {
  for (const theme of listThemes()) {
    ...
    assert.ok(
      ["low", "medium-low", "medium", "medium-high", "high"].includes(theme.formality),
      `${theme.slug}: invalid formality`,
    );
    assert.ok(["light", "dark", "mixed"].includes(theme.scheme), `${theme.slug}: invalid scheme`);
  }
});

Deno.test("theme registry getThemeSummaries returns a string with all theme slugs", () => {
...
Deno.test("theme registry isValidThemeSlug returns true for known slugs", () => {
  assert.ok(isValidThemeSlug("vanilla"));
  assert.ok(isValidThemeSlug("monochrome"));
  assert.ok(isValidThemeSlug("playful"));
});

Deno.test("theme registry isValidThemeSlug returns false for unknown slugs", () => {
  assert.equal(isValidThemeSlug("nonexistent"), false);
});

Deno.test("theme registry vanilla theme has empty fontImports", () => {
  const vanilla = getTheme("vanilla");
  assert.equal(vanilla.css.fontImports, "");
});

Deno.test("theme registry dark themes have dark backgrounds", () => {
...
Deno.test("theme registry dark themes have lighter text than background", () => {
  for (const theme of listThemes()) {
    if (theme.scheme === "dark") {
      ...
      assert.ok(
        textLum > bgLum,
        `${theme.slug}: dark theme text (${theme.css.textColor}, lum=${
          textLum.toFixed(3)
        }) should be lighter than background (${theme.css.background}, lum=${bgLum.toFixed(3)})`,
      );
    }
  }
});

Deno.test("theme registry light themes have darker text than background", () => {
  for (const theme of listThemes()) {
    if (theme.scheme === "light") {
      ...
      assert.ok(
        textLum < bgLum,
        `${theme.slug}: light theme text (${theme.css.textColor}, lum=${
          textLum.toFixed(3)
        }) should be darker than background (${theme.css.background}, lum=${bgLum.toFixed(3)})`,
      );
    }
  }
});

Deno.test("theme registry mixed themes have readable text contrast on their background", () => {
  for (const theme of listThemes()) {
    if (theme.scheme === "mixed") {
      ...
      assert.ok(
        ratio >= 3,
        `${theme.slug}: mixed theme contrast ratio ${
          ratio.toFixed(1)
        } is too low (background ${theme.css.background}, text ${theme.css.textColor})`,
      );
    }
  }
});

Deno.test("theme registry all CSS colors are valid 6-digit hex", () => {
  for (const theme of listThemes()) {
...
Deno.test("theme registry all palette colors are valid 6-digit hex", () => {
  for (const theme of listThemes()) {
...
Deno.test("theme registry dark themes have dark palette colors for backgrounds", () => {
  const velvet = getTheme("velvet-dark");
...
Deno.test("theme registry dark themes have light palette colors for text/highlights", () => {
  const velvet = getTheme("velvet-dark");
  const paletteLuminances = velvet.palette.map(relativeLuminance);
