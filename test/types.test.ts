...
import { artifactPaths, STAGE_NAMES } from "../src/utils/types.ts";

Deno.test("artifactPaths returns correct paths for a given output dir", () => {
...
Deno.test("artifactPaths handles absolute paths", () => {
...
Deno.test("artifactPaths all artifact paths end in .json", () => {
...
Deno.test("STAGE_NAMES contains all 6 stages", () => {
  assert.equal(STAGE_NAMES.length, 6);
});

Deno.test("STAGE_NAMES includes expected stage names", () => {
